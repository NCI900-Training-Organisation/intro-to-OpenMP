...
Solve a model 2D Poisson equation with Dirichlet boundary conditions

-Delta u = 2pi^2 * sin(pi x) sin(pi y)   in [0,1]^2
       u = sin(pi x) sin(pi y)           on the boundary

The problem is discretised over a uniform mesh by the finite difference
method and the resulting linear system is solved by either Jacobi or
red-black Gauss-Seidel iteration.

EXERCISE: parallelise the residual and the solver sweeps with worksharing
loops. Each sweep iterates over a 2D (i, j) index space, so the collapse(2)
clause is a natural fit. Replace each `#pragma omp FIXME` below.
The solution is in exercise6_solution.c.

Compile: gcc -fopenmp -g -Wall -O3 -lm -o exercise6 exercise6.c

Usage:   ./exercise6 size tolerance method
         (method is "Jacobi" or "Gauss-Seidel")
...
/* L2 norm of residual r = A u - f on the interior */
                          ...
                          int normalize)
{
    ...
    /* FIXME: parallelise this 2D loop; sum is a reduction variable */
    #pragma omp FIXME
    for (int i = 1; i < n - 1; ++i) {
        for (int j = 1; j < n - 1; ++j) {
            const double Au =
...
/* Jacobi: every next[i][j] depends only on the *old* grid, so the whole
   interior sweep is data-parallel. */
                  ...
                  double h)
{
    ...
    while (res > tol && iter < max_iter) {
        ++iter;
        /* FIXME: parallelise this sweep (reads old grid, writes next) */
        #pragma omp FIXME
        for (int i = 1; i < n - 1; ++i) {
            for (int j = 1; j < n - 1; ++j) {
...
/* Gauss-Seidel (red-black ordering). Within one colour every updated point
   reads only points of the *other* colour, which are not being modified in
   that sweep, so each colour sweep is data-parallel. */
                       ...
                       double h)
{
    ...
    while (res > tol && iter < max_iter) {
        ++iter;
        /* Red sweep: (i+j) % 2 == 0 */
        /* FIXME: parallelise the red sweep */
        #pragma omp FIXME
        for (int i = 1; i < n - 1; ++i){
            for (int j = 1; j < n - 1; ++j)
                if (((i + j) & 1) == 0)
        ...
        /* Black sweep: (i+j) % 2 == 1 */
        /* FIXME: parallelise the black sweep */
        #pragma omp FIXME
        for (int i = 1; i < n - 1; ++i){
            for (int j = 1; j < n - 1; ++j)
                if (((i + j) & 1) == 1)
