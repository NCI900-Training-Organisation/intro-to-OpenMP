...
This is the parallel solution: each CG building block (dot product,
matrix-vector product, and the AXPY/scale vector updates) is parallelised
with a worksharing loop. The dot product uses a reduction. The CG iteration
itself stays sequential — only the vector kernels run in parallel.

Compile: gcc -fopenmp -g -Wall -O3 -lm -o exercise7_solution exercise7_solution.c

Example usage: ./exercise7_solution 1e-5 < msc04515.dat
