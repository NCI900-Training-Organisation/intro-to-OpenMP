...
This is the parallel solution: the residual and both solver sweeps are
parallelised with worksharing loops using the collapse(2) clause.

Compile: gcc -fopenmp -g -Wall -O3 -lm -o exercise6_solution exercise6_solution.c

Usage:   ./exercise6_solution size tolerance method
                  ...
                  double h)
{
    ...
    while (res > tol && iter < max_iter) {
        ++iter;
        #pragma omp parallel for collapse(2) schedule(static) default(none) shared(grid,next,rhs) firstprivate(n,h2_4)
        for (int i = 1; i < n - 1; ++i) {
            for (int j = 1; j < n - 1; ++j) {
